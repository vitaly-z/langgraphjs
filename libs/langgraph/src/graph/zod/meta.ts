...
export type InteropZodToStateDefinition<
  ...
  TShape = InferInteropZodObjectShape<T>
> = {
  [key in keyof TShape]: TShape[key] extends InteropZodType<infer V, infer U>
    ? BaseChannel<V, U>
    : never;
};
      ...
      const newShapeEntries = Object.entries(
        ...
        if (
          ...
          const strExtras = JSON.stringify({
            ...meta.jsonSchemaExtra,
            description: outputSchema.description ?? schema.description,
          });
...
export function withLangGraph<TValue, TUpdate = TValue>(
  schema: InteropZodType<TValue>,
